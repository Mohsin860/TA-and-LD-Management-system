...
namespace dbproject
{
    public partial class EditLDsProfile : Form
    {
            ...
            if (!string.IsNullOrEmpty(loggedInUsername))
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    try
                    {
                        ...
                        string query = "SELECT FName, LName, Password, Contact FROM LDs WHERE username = @Username";
        ...
        {
            this.Hide();
            LDsHome lDsHome = new LDsHome(loggedInUsername);
            lDsHome.Show();
        }
    }
}
