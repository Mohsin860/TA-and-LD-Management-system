...
namespace dbproject
{
    public partial class EditFacultyProfile : Form
    {
        ...
        public EditFacultyProfile(string username)
        {
            InitializeComponent();
            loggedInUsername = username; // Set the logged-in username
        }

        private void EditFacultyProfile_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(loggedInUsername))
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    try
                    {
                        ...
                        string query = "SELECT FName, LName, Password, Contact FROM faculty WHERE username = @Username";
            ...
            if (!string.IsNullOrEmpty(loggedInUsername))
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    try
                    {
                        ...
                        string updateQuery = "UPDATE faculty SET FName = @FName, LName = @LName, Password = @Password, Contact = @Contact WHERE username = @Username";
